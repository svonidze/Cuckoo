namespace SBeep.Private.Useful.Cuckoo.Beeper.Imp.Stuff.Help
{
    ...
    public static class FileWizard
    {
        ...
        {
            string builder = OpenFileOneLine(path);

            var serializer = new ConfigurationSerializer<T>();
            return serializer.Deserialize(builder);
        }

        public static void SaveXml<T>(T value, string path) where T : class
        {
            var serializer = new ConfigurationSerializer<T>();
        ...
        {
            using (TextWriter writer = new StreamWriter(File.Open(path, FileMode.Create))) writer.WriteLine(input);
        }

        public static IEnumerable<T> Parse<T>(string path, Func<string[], T> action) where T : class
        {
            // open file
            List<string> builder = OpenFileLine(path).ToList();
        ...
        {
            StringBuilder builder = OpenFileLine(path)
                .Aggregate(new StringBuilder(), (current, item) => current.Append(item));
